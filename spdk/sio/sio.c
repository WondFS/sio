...
static void hello_world(void) {
	...
	TAILQ_FOREACH(ns_entry, &g_namespaces, link) {
		...
		// snprintf(sequence.buf, 0x1000, "%s", "ztmnbyyyyyyyyyyyyyyyyy!\n");
		// rc = spdk_nvme_ns_cmd_write(ns_entry->ns, ns_entry->qpair, sequence.buf, 10, 1, write_complete, &sequence, 0);
		// if (rc != 0) {
		...
		sequence.is_completed = 0;
		sequence.buf = spdk_zmalloc(0x1000, 0x1000, NULL, SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
		rc = spdk_nvme_ns_cmd_read(ns_entry->ns, ns_entry->qpair, sequence.buf, 10, 1, read_complete, &sequence, 0);
		if (rc != 0) {
			fprintf(stderr, "starting read I/O failed\n");
			exit(1);
		}
		...
		// }
		// spdk_nvme_qpair_process_completions(ns_entry->qpair, 0);
		// rc = spdk_nvme_ns_cmd_read(ns_entry->ns, ns_entry->qpair, sequence.buf, 12, 1, read_complete, &sequence, 0);
		// spdk_nvme_qpair_process_completions(ns_entry->qpair, 0);
...
static void write_data(char* data, uint64_t lba, uint32_t len) {
	TAILQ_FOREACH(ns_entry, &g_namespaces, link) {
		int rc;
		sequence.is_completed = 0;
		sequence.buf = spdk_zmalloc(0x1000, 0x1000, NULL, SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
		rc = spdk_nvme_ns_cmd_read(ns_entry->ns, ns_entry->qpair, sequence.buf, lba, len, read_complete, &sequence, 0);
		if (rc != 0) {
			fprintf(stderr, "starting read I/O failed\n");
			exit(1);
		}
